...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot word count distribution with percentile trimming.")
    parser.add_argument("--config", default=str(CONFIG_PATH), help="Path to settings.yaml")
    parser.add_argument("--plot-output", default="plots/words_avg.png", help="Output plot path")
    ...
    parser.add_argument("--trim-percent", type=float, default=10.0, help="Drop longest X%% of posts")
    parser.add_argument("--untere-perzentil", type=float, default=0.0, help="Show lower percentile line")
...
def main() -> None:
    ...
    try:
        docs = list(iter_posts(store.posts, limit=args.limit, skip=args.skip, sample=args.sample))
    finally:
        store.close()
    ...
    plt.hist(trimmed, bins=args.bins, color="#4c72b0", alpha=0.8, orientation="horizontal")
    plt.axhline(cutoff, color="#c0392b", linestyle="--", label=f"p{int(cutoff_pct)}={cutoff:.1f}")
    if lower_cutoff is not None:
        plt.axhline(lower_cutoff, color="#27ae60", linestyle="--", label=f"p{int(lower_pct)}={lower_cutoff:.1f}")
    ...
    logger.info("Trimmed %s of %s posts (%.1f%%).", removed, len(word_counts), (removed / len(word_counts)) * 100)
